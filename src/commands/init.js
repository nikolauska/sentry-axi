...
import { findGitRoot, readBindingFile, validateBinding, writeRepoBinding } from "../repo.js";
...
export async function initCommand(args, runtime) {
  ...
  if (!organization || !project) throw usage("--organization and --project are required", [
    "Run `sentry-axi init --organization <slug> --project <slug>`",
  ]);
  const root = await findGitRoot(runtime.cwd);
  if (!root) throw usage("current directory is not inside a Git repository", ["Run `git init` first"]);
  ...
  const binding = await validateBinding({
    organization,
    project,
    ...(parsed["region-url"] ? { regionUrl: parsed["region-url"] } : {}),
  }, runtime);
