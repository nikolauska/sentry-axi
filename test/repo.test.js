...
import { endpointConstraints, parseOrganizations, parseProjectSlugs, readRepoBinding } from "../src/repo.js";

test("parses hosted endpoint constraints", () => {
  assert.deepEqual(endpointConstraints("https://mcp.sentry.dev/mcp/acme/web"), { organization: "acme", project: "web" });
  assert.deepEqual(endpointConstraints("https://mcp.sentry.dev/mcp"), {});
});

test("parses organization and project discovery output", () => {
  assert.deepEqual(parseOrganizations("## **acme**\n\n**Web URL:** x\n**Region URL:** https://us.sentry.io"), [
...
test("initializes a canonical repository binding", async () => {
  ...
  const runtime = {
    ...
    client: {
      ...
      callTool: async (name, args) => {
        ...
        const text = name === "find_organizations"
          ? "## **acme**\n\n**Web URL:** x\n**Region URL:** https://us.sentry.io"
          : "- **web**\n";
  ...
  assert.deepEqual(await readRepoBinding(cwd), { organization: "acme", project: "web", regionUrl: "https://us.sentry.io" });
...
test("scoped endpoint rejects a conflicting binding", async () => {
  await assert.rejects(() => initCommand(["--organization", "other", "--project", "web"], {
    cwd: "/tmp",
